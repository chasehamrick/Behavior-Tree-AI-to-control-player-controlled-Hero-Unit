...
def myBuildTree(agent):
	...
	root = makeNode(Selector, agent, "sel1")
	ifLowHP = makeNode(HitpointDaemon, agent, "ifLowHP")
	moveToBase = makeNode(MoveToTarget, agent, (agent.world.getEnemyBases(agent.getTeam())[0].getLocation()), "move")
	root.addChild(ifLowHP)
	root.addChild(moveToBase)
	retreat = makeNode(Retreat, agent, "retreat")
	ifLowHP.addChild(retreat)
	...
	killTower = makeNode(KillTower, agent, "killTower")
	killBase = makeNode(KillBase, agent, "killBase")
	ifBuffed = makeNode(BuffDaemon, agent, "ifBuffed")
	moveToBase.addChild(chaseMinion)
	moveToBase.addChild(ifBuffed)
	moveToBase.addChild(killTower)
	moveToBase.addChild(killBase)
	chaseMinion.addChild(killMinion)
	ifBuffed.addChild(killHero)
...
class KillBase(BTNode):

	### target: the base to shoot
	...
	def enter(self):
		BTNode.enter(self)
		self.agent.stopMoving()
		enemyBase = self.agent.world.getEnemyBases(self.agent.getTeam())
		for e in enemyBase:
			if distance(self.agent.getLocation(), e.getLocation()) < BIGBULLETRANGE:
				self.target = e
				return None
